...
TURN_BINS = [(1, 20), (21, 40), (41, 60), (61, 80), (81, 100)]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create 100-turn persona reversion plots.")
...
def build_bin_means(df: pd.DataFrame) -> pd.DataFrame:
    records: list[dict] = []
...
def build_delta_heatmap(df: pd.DataFrame) -> pd.DataFrame:
    binned = build_bin_means(df)
...
def save_progress_panel(bin_df: pd.DataFrame, output_dir: Path, stem: str) -> Path:
    ...
    axes[-1].axis("off")
    fig.suptitle("100-Turn Persona Trajectories by Turn Bin", y=1.02)
    fig.tight_layout()
...
def save_condition_small_multiples(bin_df: pd.DataFrame, output_dir: Path, stem: str) -> Path:
    sns.set_theme(style="whitegrid", context="talk")
...
def main() -> None:
    ...
    bin_df = build_bin_means(df)
    distance_df = build_distance_to_default(df)
    delta_df = build_delta_heatmap(df)
    save_progress_panel(bin_df, output_dir, stem)
    ...
    save_condition_small_multiples(bin_df, output_dir, stem)
